...
func (p *InstallWorkers) Prepare(config *v1beta1.Cluster) error {
	...
	p.hosts = p.Config.Spec.Hosts.Workers().Filter(func(h *cluster.Host) bool {
		return !h.Reset && !h.Metadata.NeedsUpgrade &&
			(h.Metadata.K0sRunningVersion == nil || !h.Metadata.Ready)
	})
...
func (p *InstallWorkers) After() error {
	if NoWait {
		for _, h := range p.hosts {
			if h.Metadata.K0sJoinToken != "" {
				log.Warnf(
					"%s: --no-wait given, created join tokens will remain valid for 10 minutes",
					p.leader,
				)
				break
			}
	...
	for i, h := range p.hosts {
		...
		err := p.Wet(
			p.leader,
			fmt.Sprintf("invalidate k0s join token for worker %s", h),
			func() error {
				log.Debugf("%s: invalidating join token for worker %d", p.leader, i+1)
				return p.leader.Exec(
					p.leader.Configurer.K0sCmdf(
						"token invalidate --data-dir=%s %s",
						p.leader.K0sDataDir(),
		...
		_ = p.Wet(h, "overwrite k0s join token file", func() error {
			...
			if err := h.Configurer.WriteFile(h, h.K0sJoinTokenPath(), "# overwritten by cfctl after join\n", "0600"); err != nil {
				log.Warnf(
					"%s: failed to overwrite the join token file at %s",
					h,
					h.K0sJoinTokenPath(),
				)
			}
...
func (p *InstallWorkers) Run() error {
	...
	err := p.parallelDo(p.hosts, func(h *cluster.Host) error {
		if p.IsWet() || !p.leader.Metadata.DryRunFakeLeader {
			...
			if err := retry.Times(context.Background(), 2, node.HTTPStatusFunc(h, healthz, 200, 401)); err != nil {
				return fmt.Errorf(
					"failed to connect from worker to kubernetes api at %s - check networking",
					url,
				)
			}
		} else {
	...
	for i, h := range p.hosts {
		...
		err = p.Wet(
			p.leader,
			fmt.Sprintf("generate a k0s join token for worker %s", h),
			func() error {
	...
	return p.parallelDo(p.hosts, func(h *cluster.Host) error {
		err := p.Wet(
			h,
			fmt.Sprintf("write k0s join token to %s", h.K0sJoinTokenPath()),
			func() error {
		...
		err = p.Wet(
			h,
			fmt.Sprintf(
				"install k0s worker with `%s`",
				strings.ReplaceAll(cmd, h.Configurer.K0sBinaryPath(), "k0s"),
			),
			func() error {
				return h.Exec(cmd)
			},
