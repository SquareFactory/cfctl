...
func (p *ConfigureK0s) DryRun() error {
	for _, h := range p.hosts {
		...
		case configSourceExisting:
			p.DryMsgf(
				h,
				"k0s configuration is based on an existing k0s configuration found on %s",
				p.Config.Spec.K0sLeader(),
			)
		case configSourceProvided:
			p.DryMsg(h, "k0s configuration is based on spec.k0s.config in cfctl config")
		case configSourceNodeConfig:
			p.DryMsg(
				h,
				"k0s configuration is a generated node specific config for dynamic config clusters",
			)
		}
...
func (p *ConfigureK0s) validateConfig(h *cluster.Host, configPath string) error {
	...
	var cmd string
	log.Debugf(
		"%s: comparing k0s version %s with %s",
		h,
		p.Config.Spec.K0s.Version,
		configCreateSinceVersion,
	)
...
func (p *ConfigureK0s) configureK0s(h *cluster.Host) error {
	...
	if h.Metadata.K0sRunningVersion != nil && !h.Metadata.NeedsUpgrade {
		...
		return retry.Timeout(
			...
			node.ServiceRunningFunc(h, h.K0sServiceName()),
		)
	}
