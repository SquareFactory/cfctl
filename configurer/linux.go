...
func (l *Linux) DownloadURL(h os.Host, url, destination string, opts ...exec.Option) error {
	return h.Exec(
		fmt.Sprintf(`curl -sSLf -o %s %s`, shellescape.Quote(destination), shellescape.Quote(url)),
		opts...)
}
...
func (l *Linux) DownloadK0s(h os.Host, path string, version *version.Version, arch string) error {
	...
	url := fmt.Sprintf(
		"https://github.com/k0sproject/k0s/releases/download/v%[1]s/k0s-v%[1]s-%[2]s",
		v,
		arch,
	)
...
func (l *Linux) KubectlCmdf(h os.Host, dataDir, s string, args ...interface{}) string {
	return fmt.Sprintf(
		`env "KUBECONFIG=%s" %s`,
		l.KubeconfigPath(h, dataDir),
		l.K0sCmdf(`kubectl %s`, fmt.Sprintf(s, args...)),
	)
}
...
func (l *Linux) HTTPStatus(h os.Host, url string) (int, error) {
	output, err := h.ExecOutput(
		fmt.Sprintf(`curl -kso /dev/null --connect-timeout 20 -w "%%{http_code}" "%s"`, url),
	)
	if err != nil {
		return -1, err
	}
...
func (l *Linux) PrivateInterface(h os.Host) (string, error) {
	output, err := h.ExecOutput(
		fmt.Sprintf(
			`%s; (ip route list scope global | grep -E "\b(172|10|192\.168)\.") || (ip route list | grep -m1 default)`,
			sbinPath,
		),
	)
	if err == nil {
	...
	return "", fmt.Errorf(
		"failed to detect a private network interface, define the host privateInterface manually (%s)",
		err.Error(),
	)
}
...
func (l *Linux) PrivateAddress(h os.Host, iface, publicip string) (string, error) {
	output, err := h.ExecOutput(
		fmt.Sprintf("%s ip -o addr show dev %s scope global", sbinPath, iface),
	)
	if err != nil {
		return "", fmt.Errorf(
			"failed to find private interface with name %s: %s. Make sure you've set correct 'privateInterface' for the host in config",
			iface,
			output,
		)
	}
