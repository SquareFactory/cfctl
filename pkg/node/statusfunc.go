	...
	return func(_ context.Context) error {
		output, err := h.ExecOutput(
			h.Configurer.KubectlCmdf(
				h,
				h.K0sDataDir(),
				"get node -l kubernetes.io/hostname=%s -o json",
				h.Metadata.Hostname,
			),
			exec.HideOutput(),
			exec.Sudo(h),
...
func K0sDynamicConfigReadyFunc(h *cluster.Host) retryFunc {
	return func(_ context.Context) error {
		output, err := h.ExecOutput(
			h.Configurer.K0sCmdf(
				"kubectl --data-dir=%s -n kube-system get event --field-selector involvedObject.name=k0s -o json",
				h.K0sDataDir(),
			),
			exec.Sudo(h),
		)
	...
	return func(_ context.Context) error {
		output, err := h.ExecOutput(
			h.Configurer.KubectlCmdf(
				h,
				h.K0sDataDir(),
				"-n kube-system get events --field-selector reason=Scheduled -o json",
			),
			exec.HideOutput(),
			exec.Sudo(h),
		)
		...
		for _, e := range events.Items {
			if e.EventTime.Before(since) {
				log.Tracef(
					"%s: skipping prior event for %s: %s < %s",
					h,
...
func SystemPodsRunningFunc(h *cluster.Host) retryFunc {
	return func(_ context.Context) error {
		output, err := h.ExecOutput(
			h.Configurer.KubectlCmdf(h, h.K0sDataDir(), "-n kube-system get pods -o json"),
			exec.HideOutput(),
			exec.Sudo(h),
		)
		if err != nil {
		...
		for _, p := range pods.Items {
			if p.Status.Phase != "Running" {
				log.Tracef(
					"%s: pod phase '%s' - container statuses: %+v",
					h,
					p.Status.Phase,
