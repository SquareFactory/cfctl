...
func (k *K0s) validateMinDynamic() func(interface{}) error {
	return func(value interface{}) error {
		...
		if k.Version != nil && !k.Version.IsZero() && !k0sDynamicConfigSince.Check(k.Version) {
			return fmt.Errorf(
				"dynamic config only available since k0s version %s",
				k0sDynamicConfigSince,
			)
		}
...
func (k *K0s) GenerateToken(h *Host, role string, expiry time.Duration) (string, error) {
	...
	err := retry.Timeout(context.TODO(), retry.DefaultTimeout, func(_ context.Context) error {
		output, err := h.ExecOutput(
			h.Configurer.K0sCmdf("token create %s", k0sFlags.Join()),
			exec.HideOutput(),
			exec.Sudo(h),
		)
		if err != nil {
...
func (k *K0s) GetClusterID(h *Host) (string, error) {
	return h.ExecOutput(
		h.Configurer.KubectlCmdf(
			h,
			h.K0sDataDir(),
			"get -n kube-system namespace kube-system -o template={{.metadata.uid}}",
		),
		exec.Sudo(h),
	)
}
